...
def find_relevant_decision(store: StateStore, pr_number: str) -> str:
    ...
    pr_artifact = next((a for a in artifacts if a.get("artifact_type") == "github_pr" and str(a.get("ref", {}).get("key")) == str(pr_number)), None)
    ...
    if pr_artifact:
        ...
        for link in links:
            u = link.get("from", {})
            v = link.get("to", {})
            if (u.get("node_type") == "artifact" and u.get("id") == pr_artifact["artifact_id"]) or \
               (v.get("node_type") == "artifact" and v.get("id") == pr_artifact["artifact_id"]):
                if u.get("node_type") == "decision": return u.get("id")
                if v.get("node_type") == "decision": return v.get("id")
