...
def generate_boot_brief(store: StateStore) -> str:
    ...
    in_progress = sum(1 for wi in store.work_items.values() if wi.get("status") == "in_progress")
    done = sum(1 for wi in store.work_items.values() if wi.get("status") == "done")
    blocked = sum(1 for wi in store.work_items.values() if wi.get("status") == "blocked")
    ...
    blocker_wids = [wid for wid, wi in store.work_items.items() if wi.get("status") == "blocked"]
    if blocker_wids:
    ...
    active_wids = [wid for wid, wi in store.work_items.items() if wi.get("status") == "in_progress"]
    if active_wids:
    ...
    todo_wids = [wid for wid, wi in store.work_items.items() if wi.get("status") == "todo"]
    if todo_wids:
