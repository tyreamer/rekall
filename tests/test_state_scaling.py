...
def test_rollover_at_count_threshold(tmp_path):
    ...
    try:
        ...
        for i in range(7):
            store.append_jsonl_idempotent("timeline", {"event_id": f"evt-{i}", "note": "test"}, "event_id")
        ...
        assert "streams/timeline/seg-0001.jsonl" in store.manifest["streams"]["timeline"]["segments"]
...
def test_rollover_at_size_threshold(tmp_path):
    ...
    try:
        ...
        store.append_jsonl_idempotent("timeline", {"event_id": "next", "data": "short"}, "event_id")
        ...
        store.append_jsonl_idempotent("timeline", {"event_id": "trigger", "data": "trigger"}, "event_id")
...
def test_idempotency_across_segments(tmp_path):
    ...
    try:
        ...
        store.append_jsonl_idempotent("timeline", {"event_id": "target", "val": 1}, "event_id")
        store.append_jsonl_idempotent("timeline", {"event_id": "other", "val": 2}, "event_id")
        # Rollover triggers on next write
        store.append_jsonl_idempotent("timeline", {"event_id": "trigger", "val": 3}, "event_id")
        
        # Now 'target' is in seg-0001.jsonl
        # Append 'target' again
        store.append_jsonl_idempotent("timeline", {"event_id": "target", "val": 99}, "event_id")
